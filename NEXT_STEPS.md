...
## 提示词纪律：通用规则进 system（2026-09 起，现五条）
...
- **后续增补（同一条判据：与工具无关、反复被踩）**：**Instructions**（工作区
  ...
  密钥与临时状态）；**Cleanup**（任务结束后工作区要干净——能用 bash 就把 scratch 写到
  系统 temp；必须落在工作区时放进**一个**自明名字的临时目录整目录删；**绝不删**不是
  自己造的 / git 跟踪的 / 会话日志；收尾用 `git status` 自证，只该剩下要交付的东西）

**为什么这么写（普适性）**：这几条描述的是**任何工具、任何任务**下的行为边界，
而不是"别重复调用某个工具"这类一次性补丁；将来加工具、换宿主、换模型，它们依然成立。
具体事故只作为"为什么要有这条"的证据留在本节，不进 system：

- **Scope/Economy/Evidence 的来历**：log 里那几次多余的联网调用与静默命令。
- **Cleanup 的两次实测**：① 一次真模型会话在仓库根留下三个扫描脚本
  （`_tmp_demo_scan.py` / `_tmp_demo_scan.txt` / `_tmp_scan.py`）——未跟踪，所以
  `git status` 之外没人会提醒，用户得先分辨哪些是垃圾才能看清真正的 diff；
  ② `369aa10` 的提交信息里就写着"工作区清理：删掉 8 天前的 web_search diff 残留
  `_tmp_d.txt` 与 `eval/recall/__pycache__`"——真实残留**不止"临时脚本及其输出"**，
  所以规则写**类别**（脚本/重定向产物/缓存/复制件）而不是穷举，否则射程比现实窄。
- **Cleanup 的两条硬护栏为什么必须写**：`--sessions` 默认 `.sessions`（相对**工作区**），
  而 `.sessions/` 在 `.gitignore` 里——会话日志（唯一事实源）恰好是"带 log 字样、躺在
  工作区根、`git status` 里还看不见"的东西。只写"跑完清理"而不写"绝不删什么"，就是在
  让模型去判断一件后果不可逆的事；"我清干净了"本身也无法自证，所以换成 `git status`
  这种可核对的判据。反面证据：`85a39eb` 早就写着"临时脚本写在系统 temp、跑完即删，
  不进仓库"——**只写在文档里挡不住**，这正是把它提成 system 规则的理由。

**测试**：`test_system_prompt_carries_general_discipline` 守住"五条纪律确实渲染进
system，且排在工具段之前"——否则规则会退化成"文档里的建议"。Cleanup 额外钉住**可执行
内容与安全半边**（自明临时目录 / 绝不删不是自己造的 · git 跟踪的 · 会话日志 /
`git status`）：变异测试实测，只钉一个四词引子时，把整句砍到 32 字符、或删掉"绝不删"
那一句，用例**照样全绿**。
